...
| **Academic Headline / Subtitle** | `_pages/about.md` (`subtitle`) & `_data/profile.yml` (`headline`)                  | E.g. _"BSc in Electrical & Electronic Engineering · Communication & Signal Processing · BUET"_.                               |
...
| **Academic Biography**           | `_data/profile.yml` (`bio_p1`, `bio_p2`)                                           | 2 concise academic paragraphs highlighting research background and focus.                                                     |
| **Bio Highlights (Emphasis)**    | `_data/profile.yml` (`<span class="bio-highlight">`)                               | Styled with gold emphasis token (`#F2C879`).                                                                                  |
| **Two-Column Info Grid**         | `_pages/about.md`, `assets/css/theme.css`                                          | 2 columns on desktop (Interests on left, Education on right); stacks to 1 column on mobile screens ($\le 768\text{px}$).      |
| **Interests List**               | `_data/profile.yml` (`interests`)                                                  | 5 concise bulleted items with triangle markers (`▹`).                                                                         |
...
| **BUET Thesis Title**              | `_data/education.yml` (`thesis`)                                                                     | "Morphology-Aware ECG Denoising Using Deep Neural Networks for Arrhythmia Detection"                      |
